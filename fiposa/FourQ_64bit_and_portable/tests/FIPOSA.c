...
int main(int argc, char *argv[])
{
    ...
    size_t          SCT_L1, SCT_L2, SCT_T, CHUNK_SIZE, T_f, freq, rem, freq_counter = 0, rem_counter = 0, counter;
    double          tau_F;
    ...
    if (ECCRYPTO_SUCCESS != (status = FOPAS_kg(&sk, &pk))) {
    ...
    for (epoch = 0 ; epoch < (1 << SCT_L1) ; epoch++) {
        ...
        for (iter = 0 ; iter < SCT_L2 ; iter++) {
            t0 = clock();
            if (ECCRYPTO_SUCCESS != (status = FOPAS_Sign(&sk, SCT_L1, SCT_L2,
        ...
        output_file.write((char*) sig, SCT_L2 * sizeof(signature));
    ...
    for (epoch = 0 ; epoch < (1 << SCT_L1) ; epoch++) {
        ...
        log_file.read((char*) msg, CHUNK_SIZE);
        sig_file.read((char*) sig, SCT_L2 * sizeof(signature));
        
        for (int i = 0 ; i < SCT_L2 && counter < T_f ; i++) {
            counter ++;
            sig[i].s[0] ++;
        }
        ...
        t0 = clock();
        if (ECCRYPTO_SUCCESS != (status = FOPAS_Ver(&pk, SCT_L1, SCT_L2, msg, Xi_arr[epoch], sig, valid)))
        {
        ...
        t0 = clock();
        if (ECCRYPTO_SUCCESS != (status = FOPAS_Distill(&pk, SCT_L2, sig, valid)))
        {
    ...
    // ------------------------------------------------------------------------------------------------------------------------------------------------
    char pk_path[100] = { 0 };
    printf("INFO:  Saving the public key for audit verification ...\n");
