...
#include "FIPOSLO-app.c"
#include "FIPOSLO-sgn.c"
#include <string.h>
...
int main(int argc, char *argv[])
{
    ...
    if (ECCRYPTO_SUCCESS != (status = FIPOSLO_kg(&sk, &pk))) {            /* key generation phase */
    ...
    for (epoch = 0 ; epoch < (1 << SCT_L1) ; epoch++) {
        ...
        for (iter = 0 ; iter < SCT_L2 ; iter++) {
            t0 = clock();
            if (ECCRYPTO_SUCCESS != (status = FIPOSLO_Sign(&sk, SCT_L1, SCT_L2,
    ...
    for (epoch = 0 ; epoch < (1 << SCT_L1) ; epoch++) {
        ...
        if (ECCRYPTO_SUCCESS != (status = FIPOSLO_Ver(&pk, SCT_L1, SCT_L2, msg, Xi_arr[epoch], sig, valid)))
        {
#ifdef INFO
        ...
        if (ECCRYPTO_SUCCESS != (status = FIPOSLO_Distill(&pk, SCT_L2, sig, valid)))
        {
#ifdef INFO
    ...
    if (ECCRYPTO_SUCCESS != (status = FIPOSLO_sebver(pk, 1 << SCT_L1, SCT_L1, SCT_L2, &audit_valid, path2log)))
    {
...
#ifdef INFO
    if (audit_valid == 0) {
        printf("INFO:  Verifier verification successfully finished in %.2fs\n", audit_t / (CLOCKS_PER_SEC));
    } else {
        printf("ERROR: Verifier verification failed!\n");
        goto error;
    }
#endif
