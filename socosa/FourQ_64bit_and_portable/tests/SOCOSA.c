...
#include "SOCOSA-app.c"
#include "SOCOSA-sgn.c"
...
int main(int argc, char *argv[])
{
    ...
    if (ECCRYPTO_SUCCESS != (status = SOPAS_kg(&sk, &pk, SCT_L1, SCT_L2))) {            /* key generation phase */
    ...
    for (epoch = 0 ; epoch < (1 << SCT_L1) ; epoch++) {
        ...
        out_file.write((char*) &sig[SCT_L2-1], sizeof(signature));
    ...
    for (epoch = 0 ; epoch < (1 << SCT_L1) ; epoch++) {
        log_file.read((char*) msg, CHUNK_SIZE);                         /* load the logs for the i^th epoch */
        sig_file.read((char*) &sig_v, sizeof(signature));         /* load the signatures for the i^th epoch */
    ...
    if (ECCRYPTO_SUCCESS != (status = SOPAS_AuditVer(pk, 1 << SCT_L1, SCT_L1, SCT_L2, &audit_valid, path2log)))
    {
